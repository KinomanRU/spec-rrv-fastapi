...
async def login_user(session: db_dependency, user_data: UserLogin = Body()):
    return await session.get_one(Users, 1)
...
async def login_user(session: db_dependency, req: Request):
